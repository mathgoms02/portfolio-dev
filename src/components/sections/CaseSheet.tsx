...
export function CaseSheet({ project, repo, onClose }: Props) {
  ...
  const blocks = [
    { label: ui.common.problem[locale], body: project.problem ? [t(project.problem)] : null },
    { label: ui.common.approach[locale], body: project.approach ? t(project.approach) : null },
    { label: ui.common.outcome[locale], body: project.outcome ? t(project.outcome) : null },
  ].filter((b) => b.body && b.body.length > 0)

  return (
    <div className={styles.overlay} role="dialog" aria-modal="true" aria-label={project.name}>
      <button type="button" className={styles.scrim} onClick={onClose} aria-label={ui.common.close[locale]} />
          ...
          {project.links.length === 0 && project.restricted ? (
            <p className={styles.restricted}>
              <svg viewBox="0 0 10 12" width="9" height="11" aria-hidden="true">
                <path d="M1 5V3.5a3 3 0 0 1 6 0V5" fill="none" stroke="currentColor" strokeWidth="1.1" />
                <rect x="0.5" y="5" width="9" height="6.5" fill="none" stroke="currentColor" strokeWidth="1.1" />
              </svg>
              {t(project.restricted)}
            </p>
          ) : null}
          {project.links.map((link) => (
            <a key={link.href} href={link.href} target="_blank" rel="noreferrer" className={styles.link}>
