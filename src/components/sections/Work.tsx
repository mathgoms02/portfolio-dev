      ...
      <div className="wrap">
        ...
        <ol className={styles.list}>
          {projects.map((project, i) => {
            const repo = project.repo ? repos[project.repo.toLowerCase()] : undefined
            return (
              <Reveal
                as="li"
                key={project.id}
                ...
                >
                  ...
                  <span className={styles.main}>
                    ...
                    <span className={styles.stack}>
                      ...
                      {project.stack.length > 6 ? (
                        <span className={styles.more}>+{project.stack.length - 6}</span>
                      ) : null}
                    </span>
                  </span>
                  ...
                  <span className={styles.arrow} aria-hidden="true">
                    <svg viewBox="0 0 24 14" width="24" height="14">
                      <path d="M0 7h22M16 1l6 6-6 6" fill="none" stroke="currentColor" strokeWidth="1.4" />
                    </svg>
                  </span>
                </button>
              </Reveal>
        ...
        <p className={styles.provenance}>
          <span className={styles.dot} data-live={provenance !== 'snapshot' || undefined} aria-hidden="true" />
          {provenance === 'snapshot'
            ? locale === 'pt' ? 'dados do GitHub em cache local' : 'GitHub data from local cache'
